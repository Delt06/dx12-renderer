...
void PointLightPso::Draw(CommandList& commandList, const XMMATRIX viewMatrix, const XMMATRIX viewProjectionMatrix,
                         const PointLight& pointLight, const float scale) const
{
	...
	matricesCb.Compute(worldMatrix, viewMatrix, viewProjectionMatrix);
...
PointLightPso::PointLightPso(const ComPtr<ID3D12Device2> device, CommandList& commandList)
{
	...
	ThrowIfFailed(D3DReadFileToBlob(L"LightingDemo_PointLight_VertexShader.cso", &vertexShaderBlob));

	ComPtr<ID3DBlob> pixelShaderBlob;
	ThrowIfFailed(D3DReadFileToBlob(L"LightingDemo_PointLight_PixelShader.cso", &pixelShaderBlob));
